...
func insertGemfileVersion(setupCtx acceptance.SetupContext) error {
	...
	gemfileNew := strings.ReplaceAll(string(gemfileOld), "$RUNTIME_VERSION", "2.6.0")
...
func insertGemsRbVersion(setupCtx acceptance.SetupContext) error {
	...
	gemsRbNew := strings.ReplaceAll(string(gemsRbOld), "$RUNTIME_VERSION", "2.6.0")
...
func TestAcceptance(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.App, func(t *testing.T) {
			...
			tc.Env = append(tc.Env, "GOOGLE_RUNTIME=ruby26", "X_GOOGLE_TARGET_PLATFORM=gae")
